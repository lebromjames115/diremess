...
async function main() {
  ...
  // --- DM disappearing-mode changes: a "control" message (used by
  // app.js's selectDisappearing/handleIncomingControl) rides through the
  // exact same relay as a normal message. There's no dedicated server
  // endpoint for this in a DM (unlike the group version above) — either
  // side can send one, and this just proves the plumbing round-trips like
  // any other message: opaque ciphertext in, opaque ciphertext out, and
  // the JSON payload decrypts back exactly as sent.
  const controlPayload = JSON.stringify({ v: 1, kind: 'control', action: 'set-disappearing', mode: '24h' });
  const controlEnvelope = await encryptText(aliceSharedKey, controlPayload);
  r = await jsonFetch('/api/send', { method: 'POST', token: aliceToken, body: { to: bobName, ciphertext: controlEnvelope.ciphertext, iv: controlEnvelope.iv } });
  assert.strictEqual(r.status, 200, 'a DM disappearing-mode control message should send like any other message');

  r = await jsonFetch('/api/poll', { token: bobToken });
  assert.strictEqual(r.status, 200);
  assert.strictEqual(r.data.messages.length, 1, 'bob should receive exactly the one control message');
  const bobControlDecrypted = await decryptText(bobSharedKey, r.data.messages[0].ciphertext, r.data.messages[0].iv);
  const bobControlParsed = JSON.parse(bobControlDecrypted);
  assert.strictEqual(bobControlParsed.kind, 'control');
  assert.strictEqual(bobControlParsed.action, 'set-disappearing');
  assert.strictEqual(bobControlParsed.mode, '24h', "bob's decrypted control message must carry the mode alice set");
  ...
  // ================= DISAPPEARING MESSAGES (group setting, owner-only) =================
  assert.strictEqual(group.disappearing, 'instant', 'a new group should default to disappearing:instant');

  // 'off' was removed — there is no permanent-storage option anymore
  r = await jsonFetch(`/api/groups/${group.id}/disappearing`, { method: 'POST', token: aliceToken, body: { mode: 'off' } });
  assert.strictEqual(r.status, 400, '"off" should no longer be an accepted disappearing mode');

  // a non-owner (bob) should not be able to change it
  r = await jsonFetch(`/api/groups/${group.id}/disappearing`, { method: 'POST', token: bobToken, body: { mode: '24h' } });
  assert.strictEqual(r.status, 403, 'only the group creator should be able to change disappearing mode');

  // an invalid mode should be rejected
  r = await jsonFetch(`/api/groups/${group.id}/disappearing`, { method: 'POST', token: aliceToken, body: { mode: 'nonsense' } });
  assert.strictEqual(r.status, 400, 'an unrecognized disappearing mode should be rejected');

  // the owner (alice) can change it, and it propagates to other members
  r = await jsonFetch(`/api/groups/${group.id}/disappearing`, { method: 'POST', token: aliceToken, body: { mode: '24h' } });
  assert.strictEqual(r.status, 200, 'the group creator should be able to change disappearing mode');
  assert.strictEqual(r.data.group.disappearing, '24h');

  r = await jsonFetch('/api/poll', { token: bobToken });
  const bobDisappearingUpdate = r.data.messages.find((m) => m.system === 'group-update' && m.group.disappearing === '24h');
  assert.ok(bobDisappearingUpdate, 'other members should be notified when the creator changes disappearing mode');

  // a non-member cannot see or change it
  r = await jsonFetch(`/api/groups/${group.id}/disappearing`, { method: 'POST', body: { mode: '7d' } });
  assert.strictEqual(r.status, 401, 'changing disappearing mode without a token should be rejected');

  // fetching the group afterward should reflect the change
  r = await jsonFetch(`/api/groups/${group.id}`, { token: daveToken });
  assert.strictEqual(r.data.group.disappearing, '24h', 'the stored group record should reflect the new mode');

  // ================= CONTACTS (persisted, mirrors groups) =================
  // A plain per-user list, purely so a DM conversation resyncs on a fresh
  // browser/device -- same idea as GET /api/groups just above.
  r = await jsonFetch('/api/contacts', { token: aliceToken });
  assert.strictEqual(r.status, 200);
  assert.deepStrictEqual(r.data.contacts, [], 'alice should start with no saved contacts');

  r = await jsonFetch('/api/contacts', { method: 'POST', token: aliceToken, body: { username: bobName } });
  assert.strictEqual(r.status, 200, 'saving a contact should succeed');
  assert.ok(r.data.contacts.includes(bobName), "alice's contacts should include bob");

  // adding the same contact twice should not duplicate it
  r = await jsonFetch('/api/contacts', { method: 'POST', token: aliceToken, body: { username: bobName } });
  assert.strictEqual(r.status, 200);
  assert.strictEqual(r.data.contacts.filter((c) => c === bobName).length, 1, 'contact should not be duplicated');

  // contacts are per-user -- bob's list is separate from alice's
  r = await jsonFetch('/api/contacts', { token: bobToken });
  assert.strictEqual(r.status, 200);
  assert.ok(!r.data.contacts.includes(bobName), "bob's own contact list should not include himself");

  // saving a nonexistent user as a contact should fail
  r = await jsonFetch('/api/contacts', { method: 'POST', token: aliceToken, body: { username: `nobody_${suffix}` } });
  assert.strictEqual(r.status, 400, 'saving an unknown user as a contact should be rejected');

  // saving yourself as a contact should fail
  r = await jsonFetch('/api/contacts', { method: 'POST', token: aliceToken, body: { username: aliceName } });
  assert.strictEqual(r.status, 400, 'saving yourself as a contact should be rejected');

  // contacts require auth
  r = await jsonFetch('/api/contacts', { method: 'POST', body: { username: bobName } });
  assert.strictEqual(r.status, 401, 'saving a contact without a token should be rejected');
  r = await jsonFetch('/api/contacts');
  assert.strictEqual(r.status, 401, 'reading contacts without a token should be rejected');
