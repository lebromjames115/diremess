...
const renameGroupBtn = $('rename-group-btn');
const leaveConversationBtn = $('leave-conversation-btn');
...
async function resyncFromServer() {
  ...
  if (groupsResult.status === 'fulfilled') {
    for (const g of groupsResult.value.groups) {
      addConversation('group', g.id, { name: g.name, members: g.members }, { notifyServer: false });
    }
  } else {
    console.error('Could not resync groups from server:', groupsResult.reason);
  }
  if (contactsResult.status === 'fulfilled') {
    const serverContacts = new Set(contactsResult.value.contacts);
    for (const c of contactsResult.value.contacts) {
      addConversation('dm', c, {}, { notifyServer: false });
    }
    // Backfill the other direction too: a DM contact can exist locally
    // without the server ever having heard of it — e.g. it was added
    // before this sync feature existed, or a prior syncContactToServer()
    // call failed silently (offline, dropped request). Left alone, that
    // contact would never come back after this browser's storage is
    // cleared, even though "resync everything" is exactly the point of
    // this function. Push it up now so it's covered going forward.
    for (const conv of state.conversations) {
      if (conv.type === 'dm' && !serverContacts.has(conv.id)) {
        syncContactToServer(conv.id);
      }
    }
  } else {
...
renameGroupBtn.addEventListener('click', async () => {
  if (!state.activeConv || state.activeConv.type !== 'group') return;
  const current = state.activeConv.name;
  const name = prompt('Rename group:', current);
  if (name === null) return; // cancelled
  const trimmed = name.trim();
  if (!trimmed || trimmed === current) return;
  try {
    const { group } = await api(`/api/groups/${encodeURIComponent(state.activeConv.id)}/rename`, {
      method: 'POST',
      body: { name: trimmed },
    });
    applyGroupUpdate(group);
...
// Leaves a group (server removes you from its member list) or deletes a DM
// conversation (server forgets it was one of your contacts). Either way,
// this is what keeps it from just reappearing on the next resyncFromServer()
// — removing it only from this browser's localStorage wouldn't stick, since
// the whole point of that resync is to pull it right back down again.
leaveConversationBtn.addEventListener('click', async () => {
  if (!state.activeConv) return;
  const conv = state.activeConv;
  const isGroup = conv.type === 'group';
  const label = isGroup ? conv.name : conv.id;
  const ok = confirm(
    isGroup
      ? `Leave "${label}"? It'll disappear from your contact list, and you'll stop receiving its messages. Other members stay in the group.`
      : `Delete your conversation with ${label}? This removes it and its message history from this device and your account. You can message them again later to start a new one.`
  );
  if (!ok) return;
  try {
    if (isGroup) {
      await api(`/api/groups/${encodeURIComponent(conv.id)}/leave`, { method: 'POST' });
    } else {
      await api(`/api/contacts/${encodeURIComponent(conv.id)}`, { method: 'DELETE' });
    }
  } catch (err) {
    alert(err.message);
    return;
  }
  removeConversationLocally(conv);
});

// Clears a conversation out of this browser entirely: the sidebar entry,
// its message history, and any unread flag. Only called after the server
// side of a leave/delete has already succeeded (see above) — removing it
// here first and the API call second would leave it resynced right back on
// the next login if the request then failed.
function removeConversationLocally(conv) {
  const key = convKey(conv);
  state.conversations = state.conversations.filter((c) => convKey(c) !== key);
  persistConversations();
...
function renderConversation() {
  ...
  renameGroupBtn.hidden = !isGroup;
  leaveConversationBtn.textContent = isGroup ? 'Leave group' : 'Delete conversation';
