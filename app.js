...
// form of temporary, and a message is never kept around forever. For a DM
// this is a personal, local-only display preference — each side can set
// their own, and the server never sees it. For a group it's a shared
// setting the server stores on the group record and only the creator can
// change (see POST /api/groups/:id/disappearing); this is enforced in
// renderDisappearingPanel below, not just decoration.
...
function renderDisappearingPanel() {
  ...
  disappearingIntro.textContent = isGroup
    ...
    : "Choose how long messages stick around in this chat, just for you — it doesn't affect the other side's copy. There's no permanent option; every mode is temporary.";
