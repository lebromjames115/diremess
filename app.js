...
//   'instant'  - wiped the moment it's been shown on screen once (default)
...
// There is deliberately no "off"/permanent option — every mode is some
// form of temporary, and a message is never kept around forever.
//
// This is a SHARED setting for everyone in the conversation, not a private
// display preference — changing it changes it for the other side too.
// For a group, only the creator can change it (server-enforced — see
// POST /api/groups/:id/disappearing), and the change reaches every member
// via the same group-update notification used for roster changes. For a
// DM, either side can change it, because there's no "owner" of a DM to
// restrict it to. A group's setting can live on the group record the
// server already keeps (it has to know membership to route messages
// anyway), but a DM has no equivalent shared server-side record — contacts
// are a private per-user list (see /api/contacts). So a DM's mode change
// is instead sent as a small encrypted "control" message over the exact
// same pairwise channel real messages use (see selectDisappearing and
// handleIncomingControl below): the server relays it as opaque ciphertext,
// identical to a chat message, and never learns a setting changed at all.
// Control messages are consumed immediately on arrival and never appended
// to chat history, so they aren't themselves affected by disappearing.
const DISAPPEARING_LABELS = { instant: 'Instant', 'on-close': 'On close', '24h': '24 hours', '7d': '7 days' };
...
// Guards every touchpoint below so that if this browser is ever running a
// stale index.html (deployed separately from app.js, missing these
// elements), the disappearing-messages UI just doesn't appear — instead of
// throwing and breaking core flows like opening a chat or creating a group.
const disappearingUIAvailable = !!(disappearingBtn && disappearingPanel && disappearingIntro && disappearingOptions);
...
function groupFields(group) {
  return { name: group.name, members: group.members, owner: group.owner, disappearing: group.disappearing || 'instant' };
}
...
function openConversation(conv) {
  ...
  if (disappearingUIAvailable) disappearingPanel.hidden = true;
...
function closeConversationIfDisappearing(conv) {
  if (!conv || (conv.disappearing || 'instant') !== 'on-close') return;
  store.remove(keys.history(state.username, convKey(conv)));
}

// 'instant' is the strictest mode: rather than waiting for you to leave the
// chat (like 'on-close') or for time to pass (like '24h'/'7d'), a message
// is wiped from storage the moment it's actually been displayed on screen
// once. Deliberately NOT folded into pruneDisappearing/pruneAllDisappearing
// — those run before a conversation's history is read for display (so
// pruning there would delete a message before it was ever shown), whereas
// this runs after renderConversation has already built the DOM from it.
function wipeInstantAfterRender(key) {
  const conv = state.conversations.find((c) => convKey(c) === key);
  if (!conv || (conv.disappearing || 'instant') !== 'instant') return;
  store.remove(keys.history(state.username, key));
}
...
function renderConversation() {
  ...
  if (disappearingUIAvailable) disappearingBtn.textContent = `Disappearing: ${DISAPPEARING_LABELS[conv.disappearing || 'instant']}`;
...
function renderDisappearingPanel() {
  ...
  const current = conv.disappearing || 'instant';

  disappearingIntro.textContent = isGroup
    ? (canEdit
        ? "Choose how long messages stick around for everyone in this group. There's no permanent option — every mode is temporary."
        : `Set by the group's creator (${conv.owner}) — only they can change it.`)
    : "Choose how long messages stick around in this chat. This changes it for the other side too — there's no permanent option; every mode is temporary.";

  disappearingOptions.innerHTML = '';
  for (const mode of ['instant', 'on-close', '24h', '7d']) {
...
async function selectDisappearing(mode) {
  ...
  if (conv.type === 'group') {
    ...
    // DMs have no "owner" to restrict this to and no shared server-side
    // record to store it on — so instead of a REST call, send the new mode
    // as a small encrypted control message to the other side over the same
    // pairwise channel a normal message would use (see handleIncomingControl).
    // The server just relays it as ciphertext; it's applied on both ends.
    try {
      const sharedKey = await getSharedKey(conv.id);
      const payload = JSON.stringify({ v: 1, kind: 'control', action: 'set-disappearing', mode });
      const { ciphertext, iv } = await encryptText(sharedKey, payload);
      await api('/api/send', { method: 'POST', body: { to: conv.id, ciphertext, iv } });
    } catch (err) {
      alert(`Couldn't update this for the other side: ${err.message}`);
      buttons.forEach((b) => { b.disabled = false; });
      return;
    }
...
// Distinguishes a control message (a shared-setting change, like a DM's
// disappearing mode) from an actual chat message. Control messages travel
// through the exact same encrypted channel as text/media, so the server
// never learns their content or that a setting changed — but they aren't
// chat history, so they're consumed immediately here and never reach
// parseIncomingPayload/appendHistory.
function parseControlPayload(decrypted) {
  let parsed = null;
  try { parsed = JSON.parse(decrypted); } catch { return null; }
  if (parsed && parsed.v === 1 && parsed.kind === 'control') return parsed;
  return null;
}

function handleIncomingControl(from, control) {
  if (control.action === 'set-disappearing' && Object.prototype.hasOwnProperty.call(DISAPPEARING_LABELS, control.mode)) {
    addConversation('dm', from, { disappearing: control.mode }, { notifyServer: false });
    if (state.activeConv && state.activeConv.type === 'dm' && state.activeConv.id === from) {
      state.activeConv = { ...state.activeConv, disappearing: control.mode };
      pruneDisappearing(convKey(state.activeConv));
      if (disappearingUIAvailable && !disappearingPanel.hidden) renderDisappearingPanel();
      renderConversation();
    }
    renderConversationList();
  }
...
async function handleIncoming(msg) {
  ...
  try {
    ...
    // Control messages are DM-only (groups use a server-enforced REST call
    // instead — see the big comment above DISAPPEARING_LABELS) and never
    // become a chat entry.
    if (!groupId) {
      const control = parseControlPayload(decrypted);
      if (control) {
        handleIncomingControl(from, control);
        return;
      }
    }
